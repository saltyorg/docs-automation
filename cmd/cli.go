...
func updateCLIHelp(cfg *config.Config) error {
	...
	if binaryPath == "" {
		return fmt.Errorf("no binary path configured (set cli_help.binary_path in config or use --binary flag)")
	}
	...
	if docsFile == "" {
		return fmt.Errorf("no docs file configured (set cli_help.docs_file in config)")
	}
	...
	if _, err := os.Stat(docsPath); os.IsNotExist(err) {
		return fmt.Errorf("docs file not found at %s", docsPath)
	}
	...
	if !manager.HasCLISection(doc) {
		return fmt.Errorf("document does not have CLI section markers (<!-- BEGIN %s --> / <!-- END %s -->)",
			cfg.Markers.CLI, cfg.Markers.CLI)
	}

	// Update CLI section
	if err := manager.UpdateCLISection(doc, helpContent); err != nil {
		return fmt.Errorf("updating CLI section: %w", err)
	}
